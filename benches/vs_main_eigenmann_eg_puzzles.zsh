...
if [[ -n "$RESUME" ]]; then
    ...
    caffeinate -s -i -m fastchess \
        ...
        | grep --line-buffered -v "Warning" | tee /dev/tty | grep --line-buffered -v "Started game" >> benches/diagnostics/vs_main_eigenmann_eg_puzzles/${TIMESTAMP}.txt || true
else
    TIMESTAMP=$(date +%Y%m%d_%H%M%S)
    caffeinate -s -i -m fastchess \
        -engine cmd=./target/release/Greenseer name=dev \
        -engine cmd=/tmp/Greenseer_main/target/release/Greenseer name=main \
        -each proto=uci tc=10+0.1 option.Ponder=false \
        -sprt elo0=${ELO0} elo1=${ELO1} alpha=0.05 beta=0.05 \
        -rounds 200 \
        -config outname=benches/tournament_configs/${TIMESTAMP}.json \
        -concurrency 2 \
        -log engine=false \
        -pgnout file=benches/diagnostics/pgn/vs_main_eigenmann_eg_puzzles/${TIMESTAMP}.pgn notation=san nodes=true seldepth=true nps=true hashfull=true tbhits=true pv=true timeleft=true latency=true \
