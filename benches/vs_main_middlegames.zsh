...
if [[ -n "$RESUME" ]]; then
    ...
    caffeinate -s -i -m fastchess \
        ...
        | grep --line-buffered -v "Warning" | tee /dev/tty | grep --line-buffered -v "Started game" >> benches/results/vs_main_middlegames/${TIMESTAMP}.txt || true
else
    ...
    caffeinate -s -i -m fastchess \
        ...
        -concurrency 2 \
        -log engine=false \
        -pgnout file=benches/results/pgn/vs_main_middlegames/${TIMESTAMP}.pgn notation=san nodes=true seldepth=true nps=true hashfull=true tbhits=true pv=true timeleft=true latency=true \
