...
if [[ -n "$RESUME" ]]; then
    ...
    caffeinate -s -i -m fastchess \
        ...
        | grep --line-buffered -v "Warning" | tee /dev/tty | grep --line-buffered -v "Started game" >> benches/results/vs_main_endgames/${TIMESTAMP}_quick.txt || true
else
    TIMESTAMP=$(date +%Y%m%d_%H%M%S)
    caffeinate -s -i -m fastchess \
        -engine cmd=./target/release/Greenseer name=dev \
        -engine cmd=/tmp/Greenseer_main/target/release/Greenseer name=main \
        -each proto=uci tc=1+0.01 option.Ponder=false \
        -sprt elo0=${ELO0} elo1=${ELO1} alpha=0.05 beta=0.05 \
        -rounds 100000 \
        -config outname=benches/tournament_configs/${TIMESTAMP}_quick.json \
        -concurrency 2 \
        -log engine=false \
        -pgnout file=benches/results/pgn/vs_main_endgames/${TIMESTAMP}_quick.pgn notation=san nodes=true seldepth=true nps=true hashfull=true tbhits=true pv=true timeleft=true latency=true \
