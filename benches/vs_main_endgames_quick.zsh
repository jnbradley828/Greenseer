...
caffeinate -s -i -m fastchess \
    ...
    -concurrency 2 \
    -log engine=false \
    -pgnout file=benches/results/pgn/vs_main_endgames/${TIMESTAMP}_quick.pgn notation=uci nodes=true seldepth=true nps=true hashfull=true tbhits=true pv=true timeleft=true latency=true \
    ...
    | grep --line-buffered -v "Warning" | tee /dev/tty | grep --line-buffered -v "Started game" > benches/results/vs_main_endgames/${TIMESTAMP}_quick.txt
